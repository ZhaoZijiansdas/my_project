...
class v8SegmentationPoseLoss(v8DetectionLoss):
    ...
    def __call__(self, preds, batch):
        ...
        if fg_mask.sum():
            ...
            for i in range(batch_size):
                if fg_mask[i].sum():
                    ...
                    gt_kpt = keypoints[batch_idx.view(-1) == i][idx]  # (n, 51) # (n, 17, 3)                        
        ...
        loss[2] *= self.hyp.cls  # cls gain
        loss[3] *= self.hyp.dfl  # dfl gain        
        loss[4] *= self.hyp.pose / batch_size  # pose gain
        loss[5] *= self.hyp.kobj / batch_size  # kobj gain
        return loss.sum() * batch_size, loss.detach()  # loss(box, cls, dfl)

    def single_mask_loss(self, gt_mask, pred, proto, xyxy, area):
        """Mask loss for one image."""
        pred_mask = (pred @ proto.view(self.nm, -1)).view(-1, *proto.shape[1:])  # (n, 32) @ (32,80,80) -> (n,80,80)
        # print(self.nm, pred_mask.shape, gt_mask.shape)
        loss = F.binary_cross_entropy_with_logits(pred_mask, gt_mask, reduction='none')
        return (crop_mask(loss, xyxy).mean(dim=(1, 2)) / area).mean()
